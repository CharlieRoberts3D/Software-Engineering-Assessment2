...
class veterinary_practice:
    ...
    the Veterinary Practice application and allows:
        ...
        Note, in a real systems,
        use data storage rather than
        a set of arrays and an MVC architecture.
    """
    ...
    def create_appointment(self, appointment):
        """
        Adds the appointment and
        returns the appointment ID (the ID is list index)
        """
    ...
    def stock_medication(self, medication_name, amount):
        """
        Find the medication by the given name or,
        if it does not exist, creates the medication.
            Updates the amount of mediation that is in stock.

        :param medication_name:
        The name of the medication to be created/updated.
    ...
    def prepare_prescription_for_collection(self, prescription_id):
        """
        Attempts to change the prescription status
        from being prepared to ready for collection
    ...
    def collect_prescription(self, prescription_id):
        """
        Attempts to change the prescription status
        from ready for collection to collected.
