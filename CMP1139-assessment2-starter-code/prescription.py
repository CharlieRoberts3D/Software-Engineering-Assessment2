...
class PrescriptionStatus(Enum):
    ...
    # prepare the medication before it can be collected.
    preparing_order = 1
    # after the pharmacist has prepared the medication,
    # it becomes ready for collection.
    ready_for_collection = 2
    # when the medication is restocked,
    # the status should become preparing_orderED
    out_of_stock = 3
    # end status: the medication has been collected.
    collected = 4
...
class Prescription():
    ...
    When the prescription is created:
        ...
        the prescription status is preparing_order.
        ...
        YOUR TASK: Automatically update the status to:
        preparing_order or out_of_stock when:
        The medication stock level changes.
    Once the prescription is prepared, its status becomes ready_for_collection.
    Once the prescription is collected, its status becomes collected
    ...
    def prepareForCollection(self):
        """
        If the status is preparing_order:
            the order becomes ready_for_collection
        :param self
        :return True if the order becomes ready for collection;
            otherwise, False
        """
